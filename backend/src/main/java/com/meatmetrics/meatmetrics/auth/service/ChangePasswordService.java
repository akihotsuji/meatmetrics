...
public class ChangePasswordService {
     ...
     * 
     * <p>依存関係の注入により、ユーザーリポジトリを受け取ります。
     * SpringのDIコンテナにより自動的にインスタンス化されます。</p>
     * 
     * @param userRepository ユーザー集約の永続化を担当するリポジトリ（null不可）
     * @throws IllegalArgumentException userRepositoryがnullの場合
     */
     ...
     * パスワード変更ユースケース
     * 
     * <p>認証済みユーザーのパスワードを安全に変更します。
     * 現在のパスワード確認後、新しいパスワードに更新し、変更を永続化します。</p>
     * 
     * <p>処理フロー：
     * <ol>
     *   <li>ユーザーIDでユーザー集約を取得</li>
     ...
     * </p>
     * 
     * @param userId 変更対象ユーザーのID（null不可、認証済み前提）
     * @param command パスワード変更コマンド（現在・新パスワード含む、null不可）
     * @throws NoSuchElementException 指定されたユーザーIDが存在しない場合（404相当）
    ...
    public void changePassword(Long userId, ChangePasswordCommand command) {
        // 1) ユーザー取得（存在しない場合は 404）
        User user = userRepository.findById(userId)
            .orElseThrow(() -> new NoSuchElementException("ユーザーが見つかりません"));

        // 2) 現在パスワード照合（不一致なら 401）
        boolean matches = user.getPasswordHash().matches(command.getCurrentPassword());
        if (!matches) {
        ...
        // 既存の User.changePassword(old, newHash) がある場合はそれを使う。
        // 現在の User.changePassword は (oldPassword, newPasswordHash) シグネチャ。
        // PasswordHash の生成時に強度チェックが入る点に留意。

        // 例: 
        PasswordHash newHash = new PasswordHash(command.getNewPassword());
        user.changePassword(command.getCurrentPassword(), newHash);
